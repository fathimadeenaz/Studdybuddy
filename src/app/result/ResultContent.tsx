...
const roadmap: Record<Level, string[]> = {
    Beginner: ['Basics of Math', 'Introduction to Programming', 'Basic Science'],
    Intermediate: ['Algebra', 'Data Structures', 'Physics Fundamentals'],
    Advanced: ['Calculus', 'Algorithms', 'Advanced Physics'],
};
...
const getMotivationMessage = (score: number, maxScore: number) => {
    ...
    if (score === maxScore) return "Perfect score! Amazing job!";
    if (score >= maxScore * 0.7) return "Great effort! Keep going!";
...
export default function ResultContent() {
    ...
    const maxScore = 15;
    const roadmapForLevel = roadmap[level];
    ...
    return (
        <main className="min-h-screen bg-gradient-to-br from-[#961ba0] to-[#6a0dad] text-white flex justify-center items-center p-6">
            <div className={`max-w-xl w-full border border-white/20 bg-white/10 backdrop-blur-sm p-6 rounded-2xl shadow-lg transition-all duration-700 ease-out 
        ${animate ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
                <h1 className="text-3xl font-bold mb-4 text-[#38b6ff]">🎓 Your Result</h1>

                <p className="text-lg mb-2">Level: <span className="font-semibold">{level} {getBadge(level)}</span></p>
                ...
                <div className="w-full bg-white/20 rounded-full h-4 my-3">
                    <div className="bg-[#38b6ff] h-4 rounded-full transition-all duration-500" style={{ width: `${(score / maxScore) * 100}%` }} />
                </div>
